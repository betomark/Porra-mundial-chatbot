...
def clean_pre_event(evento):
    """Clean up and normalize pre-event JSON data from SofaScore."""
...
def clean_past_event(evento):
    """Clean and normalize past event JSON data."""
...
class Event:
    """Encapsulates logic for event probability and LLM payload generation."""
    def __init__(self, event_id, equipo_local, equipo_visitante):
        """Initialize the Event object with match identifiers and team names."""
    ...
    def calcular_probabilidades_poisson(self, xg_local, xg_visitante):
        """Calculate match result probabilities using a Poisson distribution."""
    ...
    def empaquetar_contexto_para_llm(self, json_sofascore, xg_local, xg_visitante):
        """Package event and statistical context into an LLM-friendly payload."""
