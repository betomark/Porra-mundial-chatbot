...
if not rankings:
    ...
    for round in rounds["powerRankingRounds"]:
        round_id = round["id"]
        response = client.get(urls.TEAMS_POWER_RANKINGS.format(16, 58210, round_id))
        round["ranking"] = response
        with open("data/power_rankings_{round_id}.json", "w", encoding="utf-8") as f:
            json.dump(response, f, indent=4, ensure_ascii=False)

if team_list == []:
    team_list = teams.get_teams(json.load("data/standings.json"))
